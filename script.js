...
function render() {
  ...
  $("mealList").innerHTML = state.meals.map((m, i) => `
    <div class="meal">
      ${m.photo ? `<img src="${m.photo}" style="width:50px;height:50px;object-fit:cover;border-radius:10px;">` : ""}
      <div class="meal-main">
        <b>${esc(m.name)}</b>
        <small>
  ...
  document.querySelectorAll(".delete").forEach(button => {
    button.onclick = () => {
      state.meals.splice(Number(button.dataset.i), 1);
      save();
      render();
    };
  });
...
function advice(t) {
  ...
  const pro = Math.round(GOALS.protein - t.protein);
...
function fill(f) {
  ...
  $("caloriesInput").value = Math.round(Number(f.calories || 0));
  $("proteinInput").value = Number(f.protein || 0);
  $("carbsInput").value = Number(f.carbs || 0);
  $("fatInput").value = Number(f.fat || 0);
...
function fillUSDA(food) {
  ...
  $("foodName").value =
    food.description || food.lowercaseDescription || "Food";

  /*
    USDA nutrition values are normally supplied per 100g.
    We therefore populate the fields for 100g.
  */

  $("caloriesInput").value = Math.round(calories);
  $("proteinInput").value = Number(protein.toFixed(1));
  $("carbsInput").value = Number(carbs.toFixed(1));
  $("fatInput").value = Number(fat.toFixed(1));
...
function showLocalResults(results) {
  $("suggestions").innerHTML = results
    .map(
      (f, i) =>
        `<button data-local="${i}">${esc(f.name)}</button>`
    )
    .join("");

  document
    .querySelectorAll("#suggestions button[data-local]")
    .forEach(button => {
      button.onclick = () => {
        fill(results[Number(button.dataset.local)]);
      };
    });
}
...
function showUSDAResults(results) {
  ...
  document
    .querySelectorAll("#suggestions button[data-usda]")
    .forEach(button => {
      button.onclick = () => {
        fillUSDA(results[Number(button.dataset.usda)]);
      };
    });
}
...
$("foodName").oninput = e => {
  ...
  */
  ...
  /*
    Always query USDA after a short delay.
    This means foods not in foods.json can still be found.
  */
